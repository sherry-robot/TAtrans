...
def train_model(model, optimizer, train_data_loader, valid_data_loader, opt):
    ...
    model.train()
    logging.info('训练epoches：%d' % opt.epochs)
    
    for epoch in range(opt.start_epoch, opt.epochs + 1):      #     (1,11)  ,   (1,21)
        if early_stop_flag:
            break 
        for batch_i, batch in enumerate(tqdm(train_data_loader)):    #  batch_size
            total_batch += 1
            ...
            if epoch >= opt.start_checkpoint_at:
                if (opt.checkpoint_interval == -1 and batch_i == len(train_data_loader) - 1) or \
                    ...
                    if current_valid_loss < best_valid_loss:  # update the best valid loss and save the model parameters
                        ...
                        torch.save(      # save model parameters  保存模型参数
                            model.state_dict(),
...
#from：  build_data_loader. keyphrase_dataset.collate_fn_fixed_tgt.collate_fn_common

    src, src_lens, src_mask, src_oov, oov_lists, src_str, \
               trg_str, trg, trg_oov, trg_lens, trg_mask, original_indices, tlt_str,tlt,tlt_lens,tlt_mask= batch



    # print("srcc最大长度为: %d " % max(src_lens))

    max_num_oov = max([len(oov) for oov in oov_lists])  # max number of oov for each batch
    batch_size = src.size(0)
    word2idx = opt.vocab['word2idx']
    target = trg_oov if opt.copy_attention else trg    # batch_size, kp_num, max_len
    # print(target)
    ...
    if opt.fix_kp_num_len:     #  固定 KP数量
        ...
        if opt.set_loss:  # K-step target assignment
            ...
            with torch.no_grad():
                ...
                title_embed, src_embed = model.encoder(src, src_lens, src_mask, tlt, tlt_lens, tlt_mask)
                encoder_output = model.title_attention_merge(src_embed, title_embed, src_mask)
                d_start_time = time.time()
                state = model.decoder.init_state(encoder_output, src_mask)
                control_embed = model.decoder.forward_seg(state)  # encoder_output
                # max_kp_num 控制码的最大长度  20， opt.assign_steps=2
    ...
    # (8 20 6)
        ...
        # print("训练编码时间 %d ", e_lost_time)
        state = model.decoder.init_state(encoder_output, src_mask)
        e_start_time = time.time()
        control_embed = model.decoder.forward_seg(state)
        e_lost_time = time_since(e_start_time)
        print("control_embed 时间 %d ", e_lost_time)
    ...
    # print("损失计算时间：%f.2" % loss_compute_time)
    ...
    total_loss.backward()
    # print(total_loss.detach().to("cpu"))
    backward_time = time_since(start_time)
    # print("反向传播时间：%f.2" % backward_time)
