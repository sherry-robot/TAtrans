...
def evaluate_loss(data_loader, model, opt):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(data_loader):
            # src, src_lens, src_mask, src_oov, oov_lists, src_str_list,trg_str_2dlist,\
            # trg, trg_oov, trg_lens, trg_mask, _ = batch

            src, src_lens, src_mask, src_oov, oov_lists, src_str_list, trg_str_2dlist, \
            trg, trg_oov, trg_lens, trg_mask, _, tlt_str, tlt, tlt_lens, tlt_mask = batch
            ...
            if opt.fix_kp_num_len:
                # src_embed = model.encoder(src, src_lens, src_mask)
                # title_embed = model.encoder(tlt, tlt_lens, tlt_mask)
                ...
                # encoder_output = model.enc_attn(src_embed, title_attn)
                ...
                if opt.set_loss:  # reassign target
                    input_tokens = src.new_zeros(batch_size, opt.max_kp_num, opt.assign_steps + 1)    # 分配步数
                    decoder_dists = []
...
def evaluate_greedy_generator(data_loader, generator, opt):
    ...
    with torch.no_grad():
        ...
        for batch_i, batch in enumerate(data_loader):
            ...
            # src, src_lens, src_mask, src_oov, oov_lists, src_str_list,trg_str_2dlist,\
            # trg, trg_oov, trg_lens, trg_mask, original_idx_list = batch
            src, src_lens, src_mask, src_oov, oov_lists, src_str_list, trg_str_2dlist, \
                trg, trg_oov, trg_lens, trg_mask, original_idx_list , tlt_str,tlt,tlt_lens,tlt_mask=batch

            if opt.fix_kp_num_len:
                n_best_result = generator.inference(src, src_lens, src_oov, src_mask, oov_lists, word2idx,tlt, tlt_lens, tlt_mask)
