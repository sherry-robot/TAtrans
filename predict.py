...
def main(opt):
    ...
    src_file = opt.src_file    # src 路径

    tokenized_src,tokenized_tlt = read_tokenized_src_file(src_file, remove_title_eos=opt.remove_title_eos)
    # pdb.set_trace()
    # for i,batch2  in enumerate(tokenized_src):    #  得到的是 src[0]文本, token化的list
    #
    #     print(i,batch2 )
    ...
    test_data = build_interactive_predict_dataset(tokenized_src,tokenized_tlt, opt, mode=mode, include_original=True)

    torch.save(test_data, open(opt.exp_path + "/test_%s.pt" % mode, 'wb'))
    # thedata=torch.load(opt.exp_path + "/test_%s.pt" % mode)
