...
def vocab_opts(parser):
    ...
    # parser.add_argument('-bert_vocab', help='Path prefix to the "vocab.pt"  file path from preprocess.py')
...
def model_opts(parser):
    ...
    parser.add_argument('-max_kp_num', type=int, default=20,
                        help='the number of control code发s')
...
def train_opts(parser):
    ...
    parser.add_argument('-train_from', default='', type=str,
                        ...
                        path to the pretrained model's state_dict.""")      #如果从一个检查点训练，那么这是到预训练模型参数的路径。
    ...
    parser.add_argument('-batch_workers', type=int, default=0,   #0
                        help='Number of workers for generating batches')

    # Optimization options
    parser.add_argument('-epochs', type=int, default=6,              # 20
...
def predict_opts(parser):
    ...
    parser.add_argument('-seed', type=int, default=9527,
                        help="""Random seed used for the experiments
                            reproducibility.""")
    parser.add_argument('-batch_size', type=int, default=16,   #  默认20或16
                        help='Maximum batch size')
