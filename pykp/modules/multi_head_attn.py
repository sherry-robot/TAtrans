...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_mask=None, attn_mask=None, state=None):
        ...
        if key_mask is not None:     #  8,277
            _key_mask = ~key_mask[:, None, :, None].bool()  # batch,1,k_len,1
               # 取反操作，id的位置为false， pad的位置为True
            gc.collect()
        ...
        attn_weights = F.dropout(attn_weights, p=self.dropout, training=self.training)     #8,277,277,8

        # print(attn_weights.shape)   #  8,277,277,8
        # batch,q_len,n_head,head_dim
        output = torch.einsum('bqkn,bknh->bqnh', attn_weights, v)     #8,277,8,64
        output = output.reshape(batch_size, q_len, -1)     #8,277,512
        output = self.out_proj(output)          # batch,q_len,dim         #8,277,512
...
class ScaledDotProductAttention(nn.Module):
    #__init__() 构造函数，创建对象后，可自动调用它
    def __init__(self,scaled_term,attn_dropout=0.1):
        super(ScaledDotProductAttention,self).__init__()   #继承， 第一个变量是自己， 第二个是self
    #自定义模型、自定义层，通过继承  nn.Modeule 完成  （pytorch）
        self.scaled_term=scaled_term
        self.dropout=nn.Dropout(attn_dropout)
        self.spftmax=nn.Softmax(dim=-1)

    def forward(self,q,k,v,mask=None):
        attn=torch.matmul(q,k)        # 张量乘法
        attn=attn/self.scaled_term

        if mask is not None:
            attn=attn.masked_fill(mask==0,-1e10)  #将矩阵中为0的数填充为 -1e10

        attn = attn.softmax(attn)
        attn= self.droput(attn)   #以一定概率丢弃一些单元， 即。x2并不是100%和上层的每一个值做了加权
