...
class similar_attn(nn.Module):
    ...
    def forward(self, src_embed, tlt_embed, src_mask):
        ...
        #
        trans_tlt_embed2=trans_tlt_embed/self.scale
        # trans_tlt_embed = torch.div(trans_tlt_embed, self.scale, rounding_mode='trunc')
        score = torch.einsum('bqnh,bknh->bkqn', trans_src_embed, trans_tlt_embed2)  # 8,17,277 ,1
        ...
        # 一行一行做归一化
        attn = F.dropout(self.softmax(score), p=self.dropout, training=self.training) # 8,277,17
        attn = torch.where(torch.isnan(attn), torch.full_like(attn, 0), attn) # [8, 17, 277, 1]
        attn = torch.squeeze(attn, dim=3)  # 8 17 277]
        # si 与title的相关性  求和
        # attn_similar = torch.einsum('bkq,bkn->bqn', attn, tlt_embed)  # 8,277,512
        attn=attn.permute(0,2,1)
        attn_similar=torch.matmul(attn, tlt_embed)
...
class title_attention_merge(nn.Module):
    def __init__(self, d_model=512, n_head=8, dropout=0.1):
        ...
        self.d_model = d_model
        self.similar_attn = similar_attn(d_model=d_model, dropout=dropout)
    ...
    def forward(self, src_embed, tlt_embed, src_mask):
        ...
        q = self.attn.q_linear(tlt_embed).permute(1, 0, 2)
        k = self.attn.k_linear(src_embed).permute(1, 0, 2)
        v = self.attn.v_linear(src_embed).permute(1, 0, 2)
        attn_output, _ = self.attn(q, k, v, attn_mask=src_mask)

        # attn_output = self.similar_attn(src_embed, tlt_embed, src_mask)

        attn_output = self.dropout(attn_output.permute(1, 0, 2))
        encoder_output = self.line_layer(attn_output + src_embed)
