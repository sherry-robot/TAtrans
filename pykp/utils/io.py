...
#     填充pad的位置为1， id位置为0
    ...
    def _pad2d(self, input_list):
        ...
        padded_batch = torch.LongTensor(input_list)     # 转为tensor类型

        input_mask = torch.ne(padded_batch, self.word2idx[PAD_WORD]).type(torch.FloatTensor)
#trg, trg_lens, trg_mask
        return padded_batch, input_list_lens, input_mask

    def collate_fn_common(self, batches, trg=None, trg_oov=None):

        tlt_str= [b['tlt_str'] for b in batches]
        tlt= [b['tlt'] for b in batches]
        ...
        seq_pairs = sorted(zip(src_lens, src, src_oov, oov_lists, src_str, trg_str, original_indices,tlt_str,tlt),
                           key=lambda p: p[0], reverse=True)

        # 把list压缩成n个元组
        src_lens, src, src_oov, oov_lists, src_str, trg_str, original_indices, tlt_str,tlt= zip(
            *seq_pairs)

        if self.load_train:
            seq_pairs = sorted(zip(src_lens, trg, trg_oov), key=lambda p: p[0], reverse=True)

            src_lens, trg, trg_oov = zip(*seq_pairs)
        ...
        # 对titlte序列进行填充pad
        tlt,tlt_lens,tlt_mask=self._pad(tlt)
        # print(type(tlt))
        # exit()

        tlt= tlt.to(self.device)
        tlt_mask=tlt_mask.to(self.device)
        ...
        if self.load_train:
            if self.seperate_pre_ab:    # True
                ...
                for b in batches:
                    ...
                    present_targets = [ t + [self.word2idx[EOS_WORD]] + [self.word2idx[PAD_WORD]] * (self.max_kp_len - len(t) - 1)
...
def build_interactive_predict_dataset(tokenized_src,tokenized_tlt, opt, mode="one2many", include_original=True):
    ...
    tokenized_src_trg_pairs = list(zip(tokenized_src,tokenized_tlt, tokenized_trg))
...
def build_dataset(src_trgs_pairs, opt, mode='one2one', include_original=True):
    ...
    :param mode: one2one or one2many， 模式，one2one, one2many
    :param include_original: keep the original texts of source and target,保留原始文本
    :return:
    '''
    word2idx = opt.vocab['word2idx']
    return_examples = []
    oov_target = 0         #词表外短语
    max_oov_len = 0
    max_oov_sent = ''           # 最大oov的句子
    

    for idx, (source,targets,title) in enumerate(src_trgs_pairs):
        ...
        tlt =[word2idx[w] if w in word2idx and word2idx[w] < opt.vocab_size else word2idx[UNK_WORD] for w in title]
        ...
        for target in targets:     # target=1个短语
            example = {}
            if include_original:        # 默认true
                example['src_str'] = source    #tokenlist
                example['trg_str'] = target
                example['tlt_str']=title
            
            example['src'] = src                # wordid
            ...
            if any([w >= opt.vocab_size for w in trg_copy]):   # 只要有一个符合。就为True
                oov_target += 1     # 词表外词数
        ...
        if mode == 'one2many' and len(examples) > 0:
            o2m_example = {}
            keys = examples[0].keys()     #取得keys

            for key in keys:
                # startswith是否以指定字符串开头
                #or key.startswith('title')
                if key.startswith('src') or key.startswith('oov') or key.startswith('title') or key.startswith('tlt') :
            ...
            if include_original:
                ...
                assert len(o2m_example['tlt']) == len(o2m_example['tlt_str'])
            else:
...
def extend_vocab_OOV(source_words, word2idx, vocab_size, max_unk_words):
    ...
    映射源单词到ids， 包括oov单词， 并且返回一个oov 列表。 如果源文本的oov个数，超过max_ink_words，  就把这些oov用<unk>代替
    """
    src_oov = []
    oov_dict = {}
    ...
    # print(src_oov, oov_dict, oov_list)
    # exit()

    return src_oov, oov_dict, oov_list


def extend_bert_vocab_OOV(source_words, word2idx, vocab_size, max_unk_words):
    pass
